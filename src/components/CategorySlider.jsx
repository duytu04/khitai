...
/**
 * Horizontal slider for weapon categories.
 * @param {Object} props - Component props
...
const CategorySlider = ({ categories: propCategories, isLoading }) => {
  ...
  // Default categories (fallback if propCategories is empty)
  const defaultCategories = [
    { name: 'Khí tài hải quân', path: '/weapons/naval' },
    { name: 'Khí tài đặc biệt không người lái', path: '/weapons/drones' },
    { name: 'Khí tài không quân', path: '/weapons/airforce' },
    { name: 'Khí tài mặt đất', path: '/weapons/ground' },
    { name: 'Khí tài hóa học & sinh học', path: '/weapons/chemical' },
    { name: 'Khí tài chiến tranh điện tử', path: '/weapons/electronic' },
    { name: 'Khí tài vũ khí hạt nhân', path: '/weapons/nuclear' },
    { name: 'Khí tài chiến tranh đặc biệt', path: '/weapons/special' },
    { name: 'Khí tài chiến tranh mạng', path: '/weapons/cyber' },
    { name: 'Khí tài quân sự cỡ nhỏ', path: '/weapons/small-arms' },
    { name: 'Khí tài chống tăng', path: '/weapons/anti-tank' },
    { name: 'Khí tài không gian', path: '/weapons/space' },
    { name: 'Khí tài hỗ trợ chiến đấu', path: '/weapons/support' },
    { name: 'Khí tài tác chiến đặc biệt', path: '/weapons/special-ops' },
  ];
        ...
        <List sx={{ display: 'inline-flex', padding: { xs: '0 0.5rem', md: '0 1rem' }, alignItems: 'center' }}>
          {isLoading ? (
            ...
            categories.map((category) => (
              <ListItem
                key={category.path}
                sx={{ marginRight: { xs: '0.75rem', md: '1.5rem' } }}
              >
                <CategoryButton
                  onClick={() => navigate(category.path)}
                  aria-label={`Chuyển đến danh mục ${category.name}`}
                >
                  <NavLink
                    to={category.path}
                    style={{ textDecoration: 'none', color: 'inherit' }}
